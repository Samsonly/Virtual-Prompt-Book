...
interface EditRehearsalNoteProps {
...
const EditRehearsalNote: React.FC<EditRehearsalNoteProps> = ({
  activeCharacter,
  noteIndex,
}) => {
  ...
  const [dialogueWords, setDialogueWords] = useState<Word[]>(() => {
    const words = note.line.flatMap((word: Word) => [
      { ...word },
      { text: " ", format: "space" },
    ]);
  ...
  const saveLineNote = () => {
    ...
    const formattedDialogue = filteredDialogueWords.reduce<Word[]>(
      (acc, word) => {
    ...
    let updatedLineNotes = projectSaveFile.lineNotes.map((charNotes: any) => {
      const character = Object.keys(charNotes)[0];
      if (character === characterName) {
        return {
          [character]: charNotes[character].map(
            (existingNote: any, noteIdx: number) =>
              noteIdx === noteIndex
  ...
  const handleStartOfLine = (event: React.MouseEvent<HTMLDivElement>) => {
    if (addedWords) {
...
export default EditRehearsalNote;
