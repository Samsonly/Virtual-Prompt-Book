...
const LineNotes: React.FC<LineNotesProps> = ({
  characterName,
  characterDialogue,
}) => {
  ...
  const [dialogueWords, setDialogueWords] = useState<DialogueWord[]>(() => {
  ...
  const saveLineNote = () => {
    ...
    const formattedDialogue = filteredDialogueWords.reduce<DialogueWord[]>(
      (acc, word) => {
