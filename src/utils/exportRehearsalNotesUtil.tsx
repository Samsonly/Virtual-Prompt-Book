...
const exportRehearsalNotesUtil = (
  rehearsalNotes: Record<string, RehearsalNote[]>[],
  department: string
) => {
  ...
  (doc as any).autoTable({
    ...
    body: staticTableRows.map((row) =>
      staticTableColumns.map((col) => row[col.dataKey as keyof typeof row])
    ),
    columnStyles: {
  ...
  const calculateTextHeight = (textArray: Word[]) => {
    let lineCount = 1;
  ...
  const calculateStartingY = (cell: any, textHeight: number, doc: jsPDF) => {
...
export default exportRehearsalNotesUtil;
